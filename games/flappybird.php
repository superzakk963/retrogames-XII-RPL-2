...
if (isLoggedIn()) {
    try {
        $pdo = getDB();
        $stmt = $pdo->prepare("SELECT MAX(s.score) AS best FROM scores s JOIN games g ON s.game_id = g.id WHERE s.user_id = ? AND g.slug = 'flappybird' AND s.deleted_at IS NULL");
